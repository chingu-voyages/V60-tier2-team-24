  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          className="h-auto rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 shadow-sm transition-all hover:bg-white"
        >
          <ListFilter className="h-[18px] w-[18px] text-slate-500" />
          Filter
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent
      ...
      >
        ...
        {STATUS_OPTIONS.map((status) => (
          <DropdownMenuCheckboxItem
            ...
            className="[&>span:first-child]:hidden mx-2 my-0.5 rounded-lg px-3 py-2 text-sm font-medium text-slate-600 focus:bg-slate-50"
          >
            <span
